...
# 순수 로직(텍스트 처리·음성 필터)은 백엔드/UI 의존이 없는 모듈로 분리해
# 헤드리스 CI에서 단위 테스트한다(이슈 #16). 여기선 그걸 가져다 쓴다.
...
class MainWindow(QMainWindow):
    ...
    def _populate_voices(self) -> None:
        ...
        # 도구 실행(subprocess)만 여기서 하고, 파싱·필터·정렬·기본선택은 순수
        # 모듈(voices)에 위임 — 그쪽이 단위 테스트 대상이다.
        ...
        best_ident = pick_default_voice(voices)
